...
def update(drone):
    ...
    if altitude > TARGET_HEIGHT - TOL and altitude < TARGET_HEIGHT + TOL:
        _hold += drone.get_delta_time()
    if _hold >= HOLD_TIME:
        print(f"Reached height of {altitude:.2f}. Target altitude: {TARGET_HEIGHT}")
        _done = True
